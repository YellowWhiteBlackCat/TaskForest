//! Shared platform-conformance scenarios run on the real Linux host.
...
use taskmanager_platform_linux::LinuxPlatformRuntime;

const DRAIN_DEADLINE: Duration = Duration::from_secs(5);
const DRAIN_POLL: Duration = Duration::from_millis(5);

/// The Linux adapter must publish host-neutral typed outcomes from the real
/// host: every observation lane accepts submissions, the event port drains,
/// failures are attributed to `linux.*`, and live process rows satisfy the
/// shared invariants.
#[test]
fn live_runtime_conformance_publishes_host_neutral_typed_outcomes() {
    let mut client =
        PlatformClient::new(LinuxPlatformRuntime::spawn().expect("complete Linux composition"));
    ...
    assert_live_smoke_ok(&drain, "linux.").expect("Linux live smoke contract failed");

    let rows = collect_process_rows(&drain);
    assert!(
        !rows.is_empty(),
        "Linux live smoke must observe at least one process"
    );
    assert_process_rows_consistent(&rows)
        .expect("Linux live process rows must satisfy shared invariants");
}
