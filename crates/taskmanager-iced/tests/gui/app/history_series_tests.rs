...
fn series_snapshot_reuses_until_the_data_epoch_changes() {
    ...
    let first = cache.get(&shell, 4, TrendSeries::CpuUsagePercent);
    let second = cache.get(&shell, 4, TrendSeries::CpuUsagePercent);
    assert!(Rc::ptr_eq(&first, &second));

    let next_epoch = cache.get(&shell, 5, TrendSeries::CpuUsagePercent);
    assert!(!Rc::ptr_eq(&first, &next_epoch));

    let other_series = cache.get(&shell, 5, TrendSeries::MemoryUsagePercent);
