...
fn percentage_series_ceiling_is_one_hundred_regardless_of_samples() {
    assert_eq!(series_max(MetricSeries::CpuUsagePercent, &[]), PERCENT_MAX);
    assert_eq!(
        series_max(MetricSeries::GpuUsagePercent, &[200.0, 300.0]),
        PERCENT_MAX
    );
    assert_eq!(
        series_max(MetricSeries::MemoryUsagePercent, &[12.0]),
        PERCENT_MAX
    );
    assert_eq!(
        series_max(MetricSeries::DiskActiveTimePct, &[200.0]),
        PERCENT_MAX,
...
fn bytes_per_sec_series_scales_to_its_finite_peak() {
    ...
    assert_eq!(series_max(MetricSeries::DiskBytesPerSec, &[]), 0.0);
    assert_eq!(
        series_max(MetricSeries::NetworkBytesPerSec, &[0.0, 0.0]),
        0.0
    );
    // A real peak sets the ceiling so the peak sample reaches the top.
    let disk = &[1_000_000.0_f32, 5_000_000.0, 2_000_000.0];
    assert_eq!(series_max(MetricSeries::DiskBytesPerSec, disk), 5_000_000.0);
    let net = &[300.0_f32, 900.0];
    assert_eq!(series_max(MetricSeries::NetworkBytesPerSec, net), 900.0);
}
...
fn device_metric_scale_picks_the_ceiling_and_bridges_metric_series() {
    ...
    assert_eq!(
        DeviceMetricScale::from(MetricSeries::GpuUsagePercent),
        DeviceMetricScale::Percent,
        "GPU% (and battery charge %) is percentage-typed"
    );
    assert_eq!(
        DeviceMetricScale::from(MetricSeries::DiskActiveTimePct),
        DeviceMetricScale::Percent,
        "disk active-time % bridges onto the fixed percentage ceiling"
    );
    assert_eq!(
        DeviceMetricScale::from(MetricSeries::DiskBytesPerSec),
        DeviceMetricScale::BytesPerSecond {
            use_bytes: true,
            use_base2: true
        },
...
fn every_scale_variant_formats_summary_values_with_its_unit() {
    ...
    assert_eq!(
        DeviceMetricScale::from(MetricSeries::CpuTemperatureC),
        DeviceMetricScale::Celsius
    );
    assert_eq!(
        DeviceMetricScale::from(MetricSeries::CpuFrequencyMhz),
...
fn cpu_power_watts_auto_scales_to_peak_not_clamped_to_one_hundred() {
    assert_eq!(
        DeviceMetricScale::from(MetricSeries::CpuPowerW),
    ...
    assert_eq!(series_max(MetricSeries::CpuPowerW, &[]), 0.0);
    ...
    assert_eq!(series_max(MetricSeries::CpuPowerW, power), 142.0);
    ...
    assert_eq!(series_max(MetricSeries::CpuPowerW, &[42.0, 42.0]), 42.0);
}
