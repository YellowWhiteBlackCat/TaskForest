...
use super::super::perf_devices::gpu::gpu_subtitle;
...
fn gpu_summary_projects_real_values_for_a_populated_snapshot() {
    ...
    assert_eq!(gpu_title(&gpu, 0), "NVIDIA GeForce");
...
fn gpu_summary_keeps_honest_dashes_for_unavailable_fields() {
    ...
    // GPUI parity: an unresolved identity never invents a heading from the
    // device id — the neutral "GPU {index}" keeps anonymous GPUs distinct.
    assert_eq!(gpu_title(&bare, 0), "GPU 0");
    assert_eq!(gpu_subtitle(&bare), "");
    ...
    assert_eq!(gpu_title(&unnamed, 0), "GPU 0");
...
fn gpu_summary_promotes_the_pci_marketing_name_without_losing_the_fact_row() {
    ...
    assert_eq!(gpu_title(&gpu, 0), "Arc B390");
    assert!(
