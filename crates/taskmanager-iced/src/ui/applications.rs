...
pub(super) fn applications_page(
    ...
    let search: Element<'_, Message, iced::Theme, iced::Renderer> = if shell.search_active() {
        row![
    ...
    let mut header_cells: Vec<Element<'_, Message, iced::Theme, iced::Renderer>> = visible_columns
        .into_iter()
    ...
    let table_body: Element<'_, Message, iced::Theme, iced::Renderer> = if projection.is_empty() {
    ...
    let action_bar: Element<'_, Message, iced::Theme, iced::Renderer> =
        if let Some(intent) = shell.pending_batch() {
            ...
            let selected_application_root = shell
                ...
                .and_then(taskmanager_shell::ProcessRowKey::application_root);
            let selected_target_count = selected_application_root
                .and_then(|root_pid| {
                    shell.projection().processes.as_deref().map(|processes| {
                        taskmanager_application::descendant_pids(processes, root_pid).len()
                    })
                })
                .unwrap_or_else(|| shell.selected_pids().len());
            let mut actions: Vec<Element<'_, Message, iced::Theme, iced::Renderer>> = Vec::new();
            ...
            actions.push(focus::button(
                ...
                t("proc.kill"),
                Message::RequestProcessBatch(taskmanager_application::ProcessBatchAction::Kill),
                true,
            ));
            if selected_application_root.is_none() {
                actions.push(focus::button(
            ...
            actions.push(focus::button(
                ...
                t("proc.suspend"),
                Message::RequestProcessBatch(taskmanager_application::ProcessBatchAction::Suspend),
                false,
            ));
            actions.push(focus::button(
                theme_snapshot,
                ...
                t("proc.resume"),
                Message::RequestProcessBatch(taskmanager_application::ProcessBatchAction::Resume),
                false,
            ));
            if selected_application_root.is_none() {
                actions.push(focus::button(
    ...
    let ribbon = crate::saved_views::presets_ribbon(
        theme_snapshot,
        &app.saved_views,
        crate::saved_views::PresetsRibbonState {
            filter: app.process_status_filter(),
            sort: shell.process_sort.0,
            ascending: shell.process_sort.1 == taskmanager_shell::SortDir::Asc,
