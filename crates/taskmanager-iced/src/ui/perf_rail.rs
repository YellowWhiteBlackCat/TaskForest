...
use taskmanager_application::{
    PowerSupplySnapshot, SensorCenterSnapshot, SensorQuantity, SensorReading, SystemSnapshot,
};
use taskmanager_shell::history::LiveGraphHistory;
use taskmanager_theme::{Theme, tokens};

use super::UnitPrefs;
use super::perf_layout::bounded_heading;
...
/// Sparkline canvas height inside one rail card. Width is `Fill` so the line
/// tracks the card width.
const SPARK_HEIGHT: f32 = 30.0;
/// Stroke width of the rail sparkline (mirrors the trend strip).
const SPARK_STROKE_WIDTH: f32 = 1.4;
/// Heading truncation budget: the rail width is fixed, so an over-long vendor
/// brand or disk model keeps a glanceable prefix + ellipsis. The full identity
/// stays available in the detail card heading.
...
pub(crate) struct RailInputs<'a> {
    ...
    pub(crate) history: &'a LiveGraphHistory,
...
fn rail_row(
    ...
    Some(match device {
        ...
        PerfDevice::Disk(index) => {
            ...
            let samples = rail_series(inputs, cached_samples, |history| {
                history
        ...
        PerfDevice::Network(index) => {
            ...
            let samples = rail_series(inputs, cached_samples, |history| {
                history.network_bytes_per_sec_for(&nic.device_id, nic.device_generation.get())
            });
            let max = finite_peak(&samples);
            RailRow {
                device,
        ...
        PerfDevice::Gpu(index) => {
            ...
            RailRow {
                ...
                cap1,
                cap2,
                samples: rail_series(inputs, cached_samples, |history| {
                    history.gpu_usage_pct_for(gpu.device_id.as_str(), gpu.device_generation.get())
                }),
                max: PERCENT_MAX,
        ...
        PerfDevice::Battery(index) => {
            ...
            RailRow {
                ...
                cap2,
                samples: rail_series(inputs, cached_samples, |history| {
                    history.battery_capacity_pct_for(battery.id.as_str())
                }),
                max: PERCENT_MAX,
...
fn rail_series(
    ...
    Rc::from(load(inputs.history).into_boxed_slice())
}
...
fn rail_widget_key(
    ...
    app.shell.projection().system_revision.hash(&mut hasher);
    app.shell.history.revision().hash(&mut hasher);
    app.memory_use_bytes().hash(&mut hasher);
    app.memory_use_base2().hash(&mut hasher);
    app.drive_use_bytes().hash(&mut hasher);
    app.drive_use_base2().hash(&mut hasher);
    app.network_use_bytes().hash(&mut hasher);
    app.network_use_base2().hash(&mut hasher);
    selected.key().hash(&mut hasher);
    selected.index().hash(&mut hasher);
    ...
    theme_snapshot.mono_font.hash(&mut hasher);
    devices.len().hash(&mut hasher);
    for device in devices {
        device.key().hash(&mut hasher);
...
fn device_card(
    ...
    let mut content = column![
        text(bounded_heading(&heading, HEADING_MAX_CHARS))
    ...
    if !subtitle.is_empty() {
        content = content.push(
            text(bounded_heading(&subtitle, HEADING_MAX_CHARS))
                .size(f32::from(tokens::FONT_11))
                .color(muted),
        );
    }
    if !cap1.is_empty() {
        content = content.push(text(cap1).size(f32::from(tokens::FONT_11)).color(muted));
    }
    if !cap2.is_empty() {
        content = content.push(text(cap2).size(f32::from(tokens::FONT_11)).color(muted));
    }
    let spark = RailSpark {
        samples,
