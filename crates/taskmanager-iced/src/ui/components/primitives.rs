...
pub(crate) fn badge<'a>(theme: &'a Theme, label: &str, tone: BadgeTone) -> IcedElement<'a> {
    let fill = theme::color(tone.fill(theme));
    let foreground = theme::color(tone.foreground(theme));
    container(
        text(label.to_owned())
...
pub(crate) fn divider<'a>(theme: &'a Theme) -> IcedElement<'a> {
    let hairline = theme::color(theme.palette().border);
...
pub(crate) fn progress<'a>(
    ...
    let palette = theme.palette();
    let track_fill = theme::color(palette.border);
    let stripe_fill = theme::color(palette.fg_muted);
    let value_fill = theme::color(tone.fill(theme));
...
pub(crate) fn tooltip<'a>(
    ...
    let tip_fg = theme::color(theme.palette().fg);
    let surface = theme;
...
pub(crate) fn state_panel<'a>(
    ...
    let tone_fill = theme::color(tone);
    let tile_fill = theme::color(tone.with_alpha(0.12));
    let tile_border = theme::color(tone.with_alpha(0.30));
    let muted = theme::color(palette.fg_muted);
