...
/// Export aligned process-history gaps as JSON `null` instead of attempting to
/// serialize an IEEE non-finite value (which JSON cannot represent).
fn serialize_history<S>(history: &&[f32], serializer: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
...
mod csv;
pub use csv::{processes_to_csv, processes_to_html};
