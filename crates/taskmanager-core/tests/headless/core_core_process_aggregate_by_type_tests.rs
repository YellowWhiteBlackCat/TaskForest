use super::group_aggregate::aggregate_by_type_typed;
use super::*;
...
/// folding its members' CPU%/memory into typed aggregates; sorted by CPU% desc.
#[test]
fn splits_kernel_and_userspace_and_sums_resources() {
    ...
    let groups = aggregate_by_type_typed(&refs, 1);
    ...
    let kernel = groups
        .iter()
        .find(|g| g.name() == "Kernel")
        .expect("Kernel");
    assert_eq!(userspace.process_count(), 2);
    assert_eq!(userspace.cpu().current_value(), Some(&15.0));
    assert_eq!(userspace.memory().current_value(), Some(&150));
    assert_eq!(kernel.process_count(), 1);
    assert_eq!(kernel.cpu().current_value(), Some(&1.0));
