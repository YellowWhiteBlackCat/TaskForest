//! Shared platform-conformance scenarios run on the real Windows host.
...
/// The Windows adapter must publish host-neutral typed outcomes from the real
/// host: every observation lane accepts submissions, the event port drains,
/// failures are attributed to `windows.*`, and live process rows satisfy the
/// shared invariants.
#[test]
fn live_runtime_conformance_publishes_host_neutral_typed_outcomes() {
    ...
    assert_live_smoke_ok(&drain, "windows.").expect("Windows live smoke contract failed");

    let rows = collect_process_rows(&drain);
    assert!(
        !rows.is_empty(),
        "Windows live smoke must observe at least one process"
    );
    assert_process_rows_consistent(&rows)
        .expect("Windows live process rows must satisfy shared invariants");
}
