...
use taskmanager_core::{
    DeviceState, DeviceStatus, FailureKind, ProviderId, SessionControlAction, SessionId,
    SessionItem, StartupBootEvidenceSnapshot, StartupEntry, StartupEvidenceFailure,
};
#[cfg(windows)]
use taskmanager_core::{
...
fn collect_boot_evidence_windows(now_ms: u64) -> StartupBootEvidenceSnapshot {
    let query = taskmanager_windows_api::WindowsEventLogQuery {
        channel: BOOT_EVIDENCE_CHANNEL.to_string(),
        provider: None,
        event_id: Some(BOOT_EVIDENCE_BOOT_EVENT_ID),
        after_record_id: None,
    };
    match taskmanager_windows_api::query_event_log(&query, 1) {
        Ok(entries) => {
        ...
        Err(taskmanager_windows_api::WindowsApiError::PermissionDenied) => boot_evidence_degraded(
            now_ms,
...
#[cfg(windows)]
fn boot_duration_ms(entry: &taskmanager_windows_api::WindowsEventLogEntry) -> Option<u64> {
...
impl SessionInventoryProvider for WinSessionInventoryProvider {
    fn refresh(&mut self) -> Result<PartialSourceSnapshot<SessionItem>, ProviderFailure> {
        let sessions =
            taskmanager_windows_api::enumerate_sessions().map_err(map_windows_api_failure)?;
        let items = sessions
            .into_iter()
            .map(|session| {
    ...
    fn control(
        ...
        match action {
            SessionControlAction::Lock => {
                taskmanager_windows_api::lock_workstation().map_err(map_windows_api_failure)
            }
            SessionControlAction::Disconnect => {
                ...
                let session_id = raw
                    ...
                    .ok_or(ProviderFailure::IdentityChanged)?;
                taskmanager_windows_api::logoff_session(session_id).map_err(map_windows_api_failure)
            }
        }
    }
}
