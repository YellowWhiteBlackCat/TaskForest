...
//! lives here in the composition edge next to the frontend products — the same
//! layer the product `[[bin]]`s occupy — which is allowed host filesystem
//! access. On a host without a `/proc` fdinfo source, no GPU, or only
//! foreign-uid processes whose `fdinfo` is `EACCES`, the result is an honest
//! empty array — never a fabricated row and never a fabricated zero. Threshold
//! suggestions are intentionally NOT emitted here; they live on the dedicated
//! `--suggest-thresholds` path.
...
// composition-edge copy is the minimal cold-start one-shot reader; the parse
// rule is kept bit-identical to the provider's so the two paths agree on the
// kernel DRM usage-stats spec.
...
/// `/proc/<pid>/fd` + `fdinfo` for each. Only processes that hold at least one
/// DRM descriptor with readable `drm-engine-` lines produce an entry; a
/// non-GPU process, a vanished pid, or a foreign-uid process whose `fdinfo` is
/// `EACCES` is skipped — an honest absence, never a fabricated zero-valued row.
///
...
///
/// Kept as a single seam so `cli.rs` stays small and the GPU bulk wiring is
