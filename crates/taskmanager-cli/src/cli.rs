//! Shared CLI harness for the four frontend products (ADR-051).
//!
...
//! graphical shells use) but never instantiates a GUI/TUI loop. One collection
//! cycle, serialize, print to stdout, exit 0.
//!
...
/// Product shape capabilities that vary the CLI surface, injected by each
/// product binary as plain values (ADR-051) — never selected by `cfg`.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct CliCapabilities {
    /// The product renders headless terminal text-frame evidence
    /// (`--snapshot [W H]`); only the TUI product provides it today.
    pub snapshot_text: bool,
    /// The product captures its own composited window (`--capture-window`);
    /// only the Windows GPUI product provides it today.
...
pub enum CliMode {
    ...
    /// default to 120×36; products without the capability report that the
    ...
    MemorySmbios,
    /// Drive the per-feature RAPL package-power escalation (ADR-023): invoke the
    /// polkit/pkexec power helper ON DEMAND and print the typed per-package
...
///
/// A single leading flag selects the mode (the unified CLI, ADR-051):
/// `--app-id`/`-a` configures the GUI's desktop identity; `--demo` runs the
/// product UI on fixture data; `--snapshot [W H]` renders headless TUI
/// text-frame evidence; `--json`/`-j` request a JSON snapshot;
...
/// mode); `--help`/`-h` requests help; no flag launches the product UI.
...
/// string into the frontend's `WindowOptions` and no provider or persistence
/// layer sees it.
...
pub fn run_json_snapshot_with(client: PlatformClient) -> io::Result<()> {
    let json = collect_json_snapshot_from_client(client, DEFAULT_COLLECTION_TIMEOUT)
        .map_err(|error| io::Error::other(format!("--json: {error}")))?;
...
pub fn run_suggest_thresholds_with(client: PlatformClient) -> io::Result<()> {
    let json = collect_suggest_thresholds_from_client(client, DEFAULT_COLLECTION_TIMEOUT)
        .map_err(|error| io::Error::other(format!("--suggest-thresholds: {error}")))?;
    let stdout = io::stdout();
    let mut handle = stdout.lock();
    handle.write_all(json.as_bytes())?;
    handle.write_all(b"\n")?;
...
/// capability values come from the product binary, never from `cfg`.
pub fn print_help_to(
    ...
    if capabilities.snapshot_text {
        writeln!(
            writer,
            "  {binary} --snapshot [W H]  headless text-frame evidence (TUI product; default 120x36)"
        )?;
    }
    if capabilities.capture_window {
