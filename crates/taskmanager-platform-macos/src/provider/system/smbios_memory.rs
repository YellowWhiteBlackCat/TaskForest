//! Registered-pending SMBIOS memory-inventory provider (macOS side of the
...
/// Linux by the polkit/pkexec helper crossing, which does not exist on macOS,
/// so the capability publishes an honest `Unsupported` descriptor and every
/// read completes with a typed failure — never a fabricated inventory row
/// (G-05 style, ADR-019).
