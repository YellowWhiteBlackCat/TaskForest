...
pub fn composition_height(memory: &MemoryMetrics) -> u16 {
    ...
    base + super::perf_data::memory_stats_row_count(memory)
}

/// The labelled stats rows: the data layer folds every typed observation into
/// its display-ready pair; this render module only styles label and value.
fn memory_stats_lines(rows: Vec<(String, String)>, theme: TuiTheme) -> Vec<Line<'static>> {
    rows.into_iter()
...
pub fn render_memory_composition(
    ...
    let stats_rows = super::perf_data::memory_stats_row_count(memory_snapshot);
    ...
    if stats_area.height > 0 {
        frame.render_widget(
            Paragraph::new(memory_stats_lines(
                super::perf_data::memory_stats_rows(memory_snapshot, use_bytes, use_base2),
                theme,
            )),
            stats_area,
        );
