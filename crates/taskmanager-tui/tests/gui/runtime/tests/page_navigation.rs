...
use taskmanager_shell::{InfoTable, PAGE_STEP};
...
fn page_keys_move_over_the_category_tree_and_reset_detail_scroll() {
    ...
    seed_projection_fact(
        &mut app.shell,
        ProjectionSeedFact::Processes(Some((1..=25).map(process).collect())),
    );
...
fn flat_inventory_sort_keeps_the_selected_provider_identity() {
    for (page, table) in [
        (AppPage::Services, InfoTable::Services),
        (AppPage::Startup, InfoTable::Startup),
        (AppPage::Users, InfoTable::Users),
    ] {
        let mut app = crate::demo_app();
        let _ = app.apply_action(AppAction::SelectPage(page));
        let count = match table {
            InfoTable::Services => app.sorted_services().len(),
            InfoTable::Startup => app.sorted_startup_entries().len(),
            InfoTable::Users => app.sorted_sessions().len(),
        };
        assert!(count >= 2, "{page:?} fixture needs two inventory rows");
        app.selected = 1;
        let identity = match table {
            InfoTable::Services => app
                .sorted_service_at(app.selected)
                .expect("service row")
                .id
                .as_str()
                .to_owned(),
            InfoTable::Startup => app
