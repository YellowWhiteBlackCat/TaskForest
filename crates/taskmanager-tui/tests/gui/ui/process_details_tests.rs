...
fn panel_rows_join_neutral_vm_values() {
    ...
    assert_eq!(pairs[7].1, vm(ProcessDetailsField::Shared));
    assert_eq!(pairs[8].1, vm(ProcessDetailsField::AnonHugePages));
    assert_eq!(
        pairs[9].1,
        format!(
            "{} / {}",
    ...
    assert_eq!(pairs[10].1, vm(ProcessDetailsField::CpuTime));
    assert_eq!(pairs[11].1, vm(ProcessDetailsField::Nice));
    assert_eq!(pairs[12].1, vm(ProcessDetailsField::DiskReadRate));
    assert_eq!(pairs[13].1, vm(ProcessDetailsField::DiskWriteRate));
    assert_eq!(pairs[14].1, vm(ProcessDetailsField::CancelledWriteBytes));
    assert_eq!(pairs[15].1, vm(ProcessDetailsField::StartTime));
    assert_eq!(pairs[16].1, vm(ProcessDetailsField::Exe));
    assert_eq!(pairs[17].1, vm(ProcessDetailsField::Cmdline));
}
...
fn verified_start_wraps_the_vm_timestamp() {
    ...
    assert!(!pairs[15].1.contains("token verified"));
}
...
fn missing_observations_render_dashes_never_fabricated_values() {
    ...
    for index in [6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17] {
