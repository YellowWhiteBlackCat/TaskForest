...
fn partition_label_formatting() {
    ...
    let win_part = DiskPartitionFixtureBuilder::new()
        .name("C:\\".into())
        .mount_point("C:\\".into())
        .fs_type("NTFS".into())
        .build();
    ...
    let linux_part = DiskPartitionFixtureBuilder::new()
        .name("nvme0n1p2".into())
...
fn unmounted_names_collapse_to_a_compact_locale_neutral_list() {
    ...
    let first = DiskPartitionFixtureBuilder::new()
        .name("nvme0n1p3".into())
        .build();
    let second = DiskPartitionFixtureBuilder::new()
        .name("/dev/nvme0n1p4".into())
        .build();
    let windows = DiskPartitionFixtureBuilder::new()
        .name("C:\\".into())
        .build();
