...
fn spec_grid(
    ...
    hardware: &HardwareInfo,
    units: UnitPreferences,
    thermal_status_segments: &[String],
) -> Div {
    ...
    let rows = cpu_spec_rows(cpu, hardware, units, thermal_status_segments);
    let mut column = div()
        .flex()
        .flex_col()
        .flex_1()
...
pub(crate) fn cpu_spec_rows(
    ...
    // MSR lane beside the cumulative counters. One row per node so the value
    // stays inside this panel's bounded slot instead of clipping the joined
    // summary; an unreadable per-node register is the shared dash inside its
    // own segment, and an unrun lane contributes no rows at all.
    for segment in thermal_status_segments {
        rows.push((i18n::t("cpu.thermal_status").to_string(), segment.clone()));
    }
