...
fn run_production(app_id: Option<String>) {
    ...
    let Some(startup_runtime) = resolve_startup_runtime(&host) else {
        return;
    };
...
/// Resolve every native startup client the production shell needs. A runtime
/// the host refuses to start is reported through its typed failure and aborts
/// composition before any window is created, exactly like the inline
/// resolution this helper replaces.
fn resolve_startup_runtime(host: &NativeAppHost) -> Option<crate::gpui_app::StartupRuntime> {
    ...
    Some(crate::gpui_app::StartupRuntime {
        config_client,
