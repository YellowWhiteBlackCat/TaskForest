//! The single quarantined module where the neutral theme meets iced (ADR-026,
//! CORE-07).
//!
//! Compiled only under `feature = "iced"` (off by default). A third crate
//! may not write `impl From<Color> for iced_core::Color` (orphan rule),
//! which is how the iced frontend grew its own local second conversion
//! point; the binding surface here is FREE FUNCTIONS so the module owns the
//! whole token→value edge. The iced frontend consumes THIS module as its
//! only conversion source — its `theme.rs` keeps only style ASSEMBLY
//! (container/button styles over converted values), which is frontend
//! policy per ADR-026 rule 3.
//!
//! Platform compensation is shared with the gpui binding through
//! [`crate::platform`]: the two bindings may differ only through named,
//! paired decisions there, never through one-sided cfg logic (CORE-07).

#![cfg(feature = "iced")]

use crate::color::{Color, Weight};
use crate::fonts::FONT_MISANS_VF;
...
/// paints with alpha).
#[must_use]
pub fn color(c: Color) -> iced_core::Color {
...
pub const BUNDLED_UI_FONT: iced_core::Font = iced_core::Font::with_name(FONT_MISANS_VF);
...
#[must_use]
pub fn ui_font(theme: &Theme) -> iced_core::Font {
    iced_core::Font::with_name(theme.ui_font)
}
...
#[must_use]
pub fn mono_font(theme: &Theme) -> iced_core::Font {
...
#[must_use]
pub fn ui_font_weight(theme: &Theme, weight: Weight) -> iced_core::Font {
    iced_core::Font {
        family: iced_core::font::Family::Name(theme.ui_font),
...
#[must_use]
pub fn font_weight(weight: Weight) -> iced_core::font::Weight {
...
#[must_use]
pub fn font_weight_over(weight: Weight, axis: WeightCompensationAxis) -> iced_core::font::Weight {
    ...
    const LADDER: [(f32, iced_core::font::Weight); 9] = [
        (100.0, iced_core::font::Weight::Thin),
        (200.0, iced_core::font::Weight::ExtraLight),
        (300.0, iced_core::font::Weight::Light),
        (400.0, iced_core::font::Weight::Normal),
        (500.0, iced_core::font::Weight::Medium),
        (600.0, iced_core::font::Weight::Semibold),
        (700.0, iced_core::font::Weight::Bold),
        (800.0, iced_core::font::Weight::ExtraBold),
        (900.0, iced_core::font::Weight::Black),
    ];
...
#[cfg(test)]
#[path = "../tests/headless/theme_iced.rs"]
mod tests;
