...
# visible compositor mode is rejected by the acceptance path.
set -euo pipefail
...
if [ "${TM_CAPTURE_SUPERVISED:-0}" != "1" ] \
    ...
    exec timeout --kill-after=10s 30m python3 "$REPO/scripts/capture_supervisor.py" \
        --repo-root "$REPO" --frontend iced -- bash "$0" "$@"
fi
...
if [ "${TM_CAPTURE_NIRI_BACKGROUND:-1}" = "1" ] \
    ...
    command -v dbus-run-session >/dev/null 2>&1 \
        || { printf 'background Iced capture requires dbus-run-session\n' >&2; exit 2; }
...
APP="$CAPTURE_RUN_ROOT/bin/taskforest-i"
...
mkdir -p "$RUN_DIR" "$RUNTIME_DIR/config" "$RUNTIME_DIR/data" \
...
mkdir -p "$RUNTIME_DIR" "$RUN_DIR/bin"
chmod 700 "$RUNTIME_DIR"
...
case "$CAPTURE_NIRI_BACKGROUND" in
  1) ;;
  0) printf 'visible Iced capture is disabled; use the private background route\n' >&2; exit 2 ;;
  *)
...
if [ "$CAPTURE_NIRI_BACKGROUND" -eq 1 ] \
  ...
  printf 'background Iced capture requires kwin_wayland --virtual; visible mode is disabled\n' >&2
  exit 2
fi
if [ "$CAPTURE_NIRI_BACKGROUND" -eq 1 ]; then
  case "${DBUS_SESSION_BUS_ADDRESS:-}" in
    ...
    *)
      printf 'background Iced capture requires the private-session D-Bus address\n' >&2
      exit 2
      ;;
  esac
fi
...
timeout --kill-after=10s 20m python3 scripts/capture_build.py \
  --repo-root "$REPO" --source "$REPO/target/debug/taskforest-i" \
  --destination "$APP" -- cargo build --locked --quiet \
  -p taskmanager-iced --bin taskforest-i
[ -x "$APP" ] || {
...
capture_one() {
  ...
  *-zh) locale="zh" ;;
  ...
  local data_home="$RUNTIME_DIR/data-$name"
