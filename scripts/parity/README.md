...
- `resolve_frontend_evidence.py` — Layer B resolver. It compares declared
  ...
  vocabulary: `contract_tag` is an opaque required field here.
- `test_resolve_frontend_evidence.py` — standard-library self-test proving the
  ...
  cell, and malformed declarations.
...
Exit codes: `0` every declared anchor resolved; `1` dangling or invalid anchor;
`2` usage / IO / discovery error. The machine-readable report lands at
`target/cross-frontend-evidence/<run>/manifest-validation.json`.
