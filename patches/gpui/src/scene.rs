...
impl Path<Pixels> {
    ...
    /// Translate every tessellated vertex of this path by `offset`.
    ///
    /// Unlike rebuilding through [`PathBuilder`] with a transform, this keeps
    /// the already-tessellated triangles and only shifts their coordinates,
    /// so per-frame animation replays stay cheap.
    pub fn translate(&self, offset: Point<Pixels>) -> Path<Pixels> {
        Path {
            id: self.id,
            order: self.order,
            ...
            vertices: self
                .vertices
                .iter()
                .map(|vertex| vertex.translate(offset))
                .collect(),
...
impl PathVertex<Pixels> {
    ...
    /// Shift this tessellated vertex by `offset` without re-running
    /// tessellation.
    pub fn translate(&self, offset: Point<Pixels>) -> PathVertex<Pixels> {
        PathVertex {
